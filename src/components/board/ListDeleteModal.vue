<template>
  <transition name="modal-fade">
    <div class="modal-backdrop z-50" v-if="showModal" @click="closeModalOnClick">
      <div class="list-delete-modal">
        <div style="text-align: center">
          <p class="list-delete-modal_title">Delete <span class="list-delete-modal_card-name">{{list?.name}}</span>?</p>
        </div>
        <div v-if="!isRequestInProcess">
          <div style="margin-top: 24px;">
            <button class="list-delete-modal_delete-button" @click="deleteCard">
              {{ isRequestFailed ? 'Try again' : 'Yes'}}, delete this <span class="list-delete-modal_delete-button_card-name">{{list?.name}}</span>
            </button>
          </div>
          <div style="margin-top: 16px; display: flex; justify-content: center">
            <button class="list-delete-modal_close-button" @click="close">Close</button>
          </div>
        </div>
...
function open(_listIndex: number, _list: List) {
...
.list-delete-modal_title {
  color: #000000;
  font-size: 16px;
  font-weight: 400;
}
.list-delete-modal_card-name {
  color: #000000;
  font-size: 16px;
  font-weight: 700;
}

.list-delete-modal_delete-button {
  height: 28px;
  min-width: 276px;
  display: flex;
  place-items: center;
...
.list-delete-modal_delete-button_card-name {
  font-size: 14px;
  font-weight: 700;

  margin-left: 5px;
}
.list-delete-modal_delete-button:hover {
  color: #FF7C5C;
  border: 1px #FF7C5C solid;
}

.list-delete-modal_close-button {
  height: 28px;
  width: 276px;
  place-items: center;
  text-align: center;
...
.list-delete-modal_close-button:hover {
  color: #FF7C5C;
  border: 1px #FF7C5C solid;
}
</style>